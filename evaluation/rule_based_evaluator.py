...
def _score_openness(f: BehavioralFeatures) -> float:
    """Openness: vocabulary diversity, novel ideas, complex expression, curiosity."""
    return (
        0.25 * _norm("unique_word_ratio", f.unique_word_ratio)
        ...
        + 0.20 * _norm("question_ratio", f.question_ratio)
    )


def _score_conscientiousness(f: BehavioralFeatures) -> float:
    """Conscientiousness: structured thinking, systematic reasoning, thoroughness."""
    return (
        0.30 * _norm("planning_count", f.planning_count)
        + 0.25 * _norm("conditional_ratio", f.conditional_ratio)
        + 0.20 * _norm("long_sentence_ratio", f.long_sentence_ratio)
        + 0.10 * _norm("avg_words_per_turn", f.avg_words_per_turn)
...
def _score_agreeableness(f: BehavioralFeatures) -> float:
    """Agreeableness: cooperation, warmth, social attentiveness vs. conflict."""
    return (
        0.25 * _norm("acknowledgment_count", f.acknowledgment_count)
        + 0.25 * _inv(_norm("disagreement_count", f.disagreement_count))
        + 0.20 * _norm("inclusive_pronoun_ratio", f.inclusive_pronoun_ratio)
...
def _score_neuroticism(f: BehavioralFeatures) -> float:
    """Neuroticism: uncertainty, negative affect, self-focus, inconsistency."""
    return (
        0.25 * _norm("hedge_count", f.hedge_count)
        + 0.25 * _norm("emotional_word_count", f.emotional_word_count)
        + 0.15 * _norm("first_person_ratio", f.first_person_ratio)
        + 0.15 * _norm("word_count_variance", f.word_count_variance)
        + 0.10 * _inv(_norm("certainty_count", f.certainty_count))
        + 0.10 * _inv(_norm("positive_emotion_count", f.positive_emotion_count))
    )
